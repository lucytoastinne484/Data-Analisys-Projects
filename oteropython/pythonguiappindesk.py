...
def DownloadCommand360p():
    ...
    download_link.streams.filter(resolution="360p").first().download(r"C:\YTDownloads")
...
def DownloadCommand720p():
    ...
    download_link.streams.filter(resolution="720p").first().download(r"C:\YTDownloads")
...
def DownloadCommand1080p():
    ...
    download_link.streams.filter(resolution="1080p").first().download(r"C:\YTDownloads")
...
def DownloadRingtone():
    
    download_link = YouTube(str(link.get()))
    
    download_link.streams.filter(only_audio=True).second().download(r"C:\YTDownloads")
...
ringtone = Label(win_gui,text="Download your favorite ringtone here:",compound=RIGHT).place(x=50,y=150)

download_ringtone = Button(win_gui,text='Download mp3',compound=LEFT,command=DownloadRingtone)
download_ringtone.pack( ipadx=12,
 ipady=12,
 )
